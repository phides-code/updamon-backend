...
func TestComputerHandlerClientErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "POST empty required string",
			method:       "POST",
			body:         fmt.Sprintf(`{"hostname":%q,"ip":%q,"os":%q}`, "", testutil.TestComputerIP, testutil.TestComputerOS),
			wantStatus:   http.StatusBadRequest,
			wantErrorMsg: "validation failed",
			setupRepo:    panicComputerRepo,
		},
		{
			name:         "POST whitespace required string",
			method:       "POST",
			body:         fmt.Sprintf(`{"hostname":%q,"ip":%q,"os":%q}`, "   ", testutil.TestComputerIP, testutil.TestComputerOS),
			wantStatus:   http.StatusBadRequest,
			wantErrorMsg: "validation failed",
		...
		{
			name:         "POST required string too long",
			method:       "POST",
			body:         fmt.Sprintf(`{"hostname":%q,"ip":%q,"os":%q}`, strings.Repeat("a", domain.DefaultMaxStringLength+1), testutil.TestComputerIP, testutil.TestComputerOS),
			wantStatus:   http.StatusBadRequest,
			wantErrorMsg: "validation failed",
			setupRepo:    panicComputerRepo,
		},
...
func TestComputerHandlerUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "PUT empty required string",
			pathID:       validUuid,
			body:         fmt.Sprintf(`{"hostname":%q,"ip":%q,"os":%q}`, "", testutil.TestComputerIP, testutil.TestComputerOS),
			wantStatus:   http.StatusBadRequest,
			wantComputer: nil,
		...
		{
			name:         "PUT whitespace required string",
			pathID:       validUuid,
			body:         fmt.Sprintf(`{"hostname":%q,"ip":%q,"os":%q}`, "   ", testutil.TestComputerIP, testutil.TestComputerOS),
			wantStatus:   http.StatusBadRequest,
			wantComputer: nil,
		...
		{
			name:         "PUT required string too long",
			pathID:       validUuid,
			body:         fmt.Sprintf(`{"hostname":%q,"ip":%q,"os":%q}`, strings.Repeat("a", domain.DefaultMaxStringLength+1), testutil.TestComputerIP, testutil.TestComputerOS),
			wantStatus:   http.StatusBadRequest,
			wantComputer: nil,
