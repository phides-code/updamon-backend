...
func TestErrorMapping(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantMsg:    domain.ErrInvalidID.Error(),
			wantClient: true,
		},
		{
			...
			wantStatus: http.StatusBadRequest,
			wantMsg:    domain.ErrValidationFailed.Error(),
			wantClient: true,
		},
		{
			name:       "invalid json",
			...
			wantStatus: http.StatusBadRequest,
			wantMsg:    domain.ErrInvalidJSON.Error(),
			wantClient: true,
		},
		{
			name:       "not found",
			...
			wantMsg:    domain.ErrNotFound.Error(),
			wantClient: true,
		},
		{
			...
			wantMsg:    domain.ErrMethodNotAllowed.Error(),
			wantClient: true,
		},
		{
			name:       "unauthorized",
			...
			wantMsg:    domain.ErrUnauthorized.Error(),
			wantClient: true,
		},
		{
			name:       "unknown error",
		...
		},
		{
			...
			wantMsg:    domain.ErrAlreadyExists.Error(),
			wantClient: true,
		},
	}
